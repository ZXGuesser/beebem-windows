	...
	*/
	...
	if ((Address & ~3)==0xfe30) {
		return(PagedRomReg); // Report back ROMSEL - I'm sure the Beeb allows ROMSEL read.
		                     // Correct me if I'm wrong. - Richard Gellman
	}
...
static void RomWriteThrough(int Address, unsigned char Value)
{
	...
	// Solidisk SW RAM board - bank to write to is selected by User VIA
	if (SolidiskSWRAMBoardEnabled)
	{
...
void BeebWriteMem(int Address, unsigned char Value)
{
	if (MachineType == Model::B) {
		...
		if (Address >= 0x8000 && Address < 0xc000) {
			if (!SolidiskSWRAMBoardEnabled && RomWritable[ROMSEL])
			{
				Roms[ROMSEL][Address - 0x8000] = Value;
			}
			else
			{
				RomWriteThrough(Address, Value);
			}
